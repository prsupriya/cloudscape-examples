...
# Mapping of service names to Service Quotas service codes
...
def get_services_from_analysis(analysis_result):
    """
    Extract AWS services from the analysis result
    """
    services = set()
    
    # Use regex to find AWS services
    matches = re.finditer(SERVICE_REGEX, analysis_result, re.IGNORECASE)
    for match in matches:
        service = match.group(1)
        services.add(service.lower())
    
    # Look for additional services by name
...
def map_service_to_code(service):
    """
    Map a service name to its AWS pricing service code
    """
...
def build_pricing_filters(service_code, resource_config):
    ...
    if service_code == 'AmazonEC2':
        ...
        if 'storageClass' in resource_config:
        ...
        if 'instanceClass' in resource_config:
...
def map_service_to_quota_code(service):
    """
    Map a service name to its Service Quotas service code
    """
    return QUOTA_SERVICE_MAPPING.get(service.lower())
