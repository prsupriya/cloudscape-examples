...
export default function Chat() {
  ...
  // Determine if we're on the assess path
  const isAssessPath = location.pathname.includes('/assess');
  const isDesignPath = location.pathname.includes('/design');

  // Set the title based on the path
  const chatMode = isAssessPath ? "- Assess" : isDesignPath ? "- Design" : " ";
  ...
  const onPromptSend = async ({ detail: { value } }: { detail: { value: string } }) => {
    ...
    // Create the new message object - this part remains similar
    ...
    try {
      ...
      // NEW: Send message and files to API
      await ChatApiService.sendChatMessage(value, files);
      ...
      // Show response after delay - this part remains similar
      setTimeout(() => {
        const validPrompt = files.length > 0
  ...
  return (
    <div className={`chat-container ${!isVisualRefresh && 'classic'}`}>
      <FittedContainer>
        <Container
          header={<Header variant="h3">Well Architected Agent Gen AI chat {chatMode}</Header>}
          fitHeight
